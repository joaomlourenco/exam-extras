...
%––––– example answer key (40 questions) –––––
\providecommand\VERSION{A}
% \providecommand\FILE{exam}
\providecommand\FILE{Test 2}
\providecommand\SOURCE{.}
% \providecommand\COURSE{Course Name}
\providecommand\COURSE{Arquiteura de Computadores}
% \providecommand\DATE{0000-00-00}
\providecommand\DATE{2026-06-08}
\providecommand\NQUESTIONS{40}
...
\begin{tikzpicture}[remember picture,overlay,yscale=-1]  % y-axis downwards
  ...
  % global parameters
  \def\fillColor{blue!90!black}
  \def\lightgray{black!20}
  \def\radius{0.18cm}
  ...
  % Course Name (all in cm)
  \def\courseX{1.5cm}
  \def\courseY{6.65cm}
  \def\CourseNameTXT{\color{\fillColor}\textsf{\LARGE \COURSE}}

  % Date (all in cm)
  \def\dateX{1.5cm}
  \def\dateY{7.7cm}
  \def\DateTXT{\color{\fillColor}\textsf{\LARGE \DATE}}
  ...
  % Course Name
  \node[anchor=west] at ($(current page.north west)+(\courseX,\courseY)$) {\CourseNameTXT};
        
  % Date
  \node[anchor=west] at ($(current page.north west)+(\dateX,\dateY)$) {\DateTXT};
  ...
  % cover part of the number
  \draw[white,fill=white]   ($(current page.north west)+(\numX+3.32, \numY-1.14)$)   rectangle ++(3.7cm, 7.1cm);

  % cover part of the questions
  \ifnum\NQUESTIONS<26
    \draw[white,fill=white]   ($(current page.north west) +
                                    ( {\colX - (1.2 * \colStepX)},
                                      {\colY - (0.5 * \colStepY) + (\NQUESTIONS * \colStepY)} )
                            $)   rectangle ++(3.7cm, { (25 - \NQUESTIONS) * \colStepY });
    \draw[white,fill=white]   ($(current page.north west) +
                                (\colX + \colOffset - 1 * \colStepX,
                                 \colY - 0.5 * \colStepY - 0.4)
                            $)   rectangle ++(3.7cm, 25 * \colStepY + 0.4);
  \else  
  \ifnum\NQUESTIONS<50
    \draw[white,fill=white]   ($(current page.north west) +
                                    ( {\colX + \colOffset - (1.2 * \colStepX)},
                                      {\colY - (0.5 * \colStepY) + ((\NQUESTIONS - 25) * \colStepY)} )
                            $)   rectangle ++(3.7cm, { (50 - \NQUESTIONS) * \colStepY });
  \fi
  \fi

  % cover part of the questions
  % \ifnum\NQUESTIONS<50
  %   \draw[white,fill=red]   ($(current page.north west)+(\colX + \colOffset , \colY)$)   rectangle ++(3.7cm, \colStepY);
  % \fi
